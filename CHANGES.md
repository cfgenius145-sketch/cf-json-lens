...
New `{ }` icon in `#1f5fbf` on white at 16/32/48/128, generated by rendering an
inline SVG at 512px with headless Chrome and downscaling with `sips`.

This took two attempts, and the first failure is worth recording. The initial
approach wrapped the SVG in an `<img>` and screenshotted at each target size.
A pixel check caught that the 16px and 48px outputs were **pure white** —
Chrome will not render below a minimum window size — so those were rebuilt from
the 128px render. But the 128px render was itself wrong: Chrome had scaled the
`<img>`-wrapped SVG and captured only the **top-right corner of the border,
with no glyph at all**. The pixel check passed it because it only asked
"are there blue pixels", which a corner of a blue border satisfies.

The defect only surfaced on looking at the options-page screenshot, where the
logo appeared as a thin blue bar. Fixed by inlining the SVG directly in the
page with explicit dimensions, rendering once at 512px, verifying the image
visually, and downscaling from there. Lesson: a non-blank assertion is not a
