...
function getPreWithSource() {
  ...
  if (childNodes.length > 1 && allTextNodes(childNodes)) {
    if (process.env.NODE_ENV === 'development') {
      console.debug("[CF JSON Lens] Loaded from a multiple text nodes, normalizing");
    }
  ...
  if (nodeName === "#text" && textContent.trim().length > 0) {
    if (process.env.NODE_ENV === 'development') {
      console.debug("[CF JSON Lens] Loaded from a text node, this might have returned content-type: text/html");
    }
