...
function oversizedJSON(pre, options, outsideViewer) {
  ...
  if (process.env.NODE_ENV === 'development') {
    console.debug("[CF JSON Lens] JSON size: " + loaded + " kbytes");
    console.debug("[CF JSON Lens] Max JSON size: " + accepted + " kbytes");
    console.debug("[CF JSON Lens] " + jsonSize + " > " + maxJsonSize + " = " + isOversizedJSON);
  }

  if (isOversizedJSON) {
    console.warn(
      "[CF JSON Lens] Content not highlighted due to oversize. " +
    ...
    message.innerHTML = "[CF JSON Lens] Content not highlighted due to oversize. " +
    ...
    return contentExtractor(pre, options).
      ...
      then(function(value) {
        ...
        if (options.addons.autoHighlight) {
          ...
          console.warn(
            "[CF JSON Lens] You are seeing the raw version because you configured the " +
