...
function renderVersion() {
  ...
  versionLink.href = "https://github.com/cfgenius145-sketch/cf-json-lens";
}

function onLoaded() {
  // Storage is asynchronous under Manifest V3.
...
function render(currentOptions) {
  renderVersion();
  ...
  bindSaveButton([addonsEditor, structureEditor, styleEditor], function(options) {
    if (!isValidJSON(options.addons)) {
      ...
      Storage.save(options).then(function() {
        ...
        sweetAlert("Ops!", "Could not save options: " + e.message, "error");
      });
    }
  });
}
